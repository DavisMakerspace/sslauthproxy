...
class SSLAuthServer < OpenSSL::SSL::SSLServer
  def initialize(server, server_key, server_cert, client_ids, verify_mode=OpenSSL::SSL::VERIFY_FAIL_IF_NO_PEER_CERT)
    ssl_context = OpenSSL::SSL::SSLContext.new
    ssl_context.key = server_key
    ssl_context.cert = server_cert
    ssl_context.verify_mode = OpenSSL::SSL::VERIFY_PEER | verify_mode
